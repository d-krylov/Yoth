...
template <std::integral T>
YOTH_HOST_DEVICE inline T FMA(T a, T b, T c) {
  return a * b + c;
}
...
YOTH_HOST_DEVICE inline auto SumOfProducts(TA a, TB b, TC c, TD d) {
  auto cd = c * d;
...
template <typename TA, typename TB, typename TC, typename TD>
YOTH_HOST_DEVICE inline auto DifferenceOfProducts(TA a, TB b, TC c, TD d) {
  auto cd = c * d;
...
template <std::floating_point T>
YOTH_HOST_DEVICE inline CompensatedFloat<T> TwoProduct(T a, T b) {
  auto ab = a * b;
...
template <std::floating_point T>
YOTH_HOST_DEVICE inline CompensatedFloat<T> TwoSum(T a, T b) {
  auto s = a + b;
  auto d = s - a;
...
template <std::floating_point T>
YOTH_HOST_DEVICE inline CompensatedFloat<T> InnerProductCompensated(T a, T b) {
  return TwoProduct(a, b);
}

template <std::floating_point F, std::floating_point... T>
YOTH_HOST_DEVICE inline CompensatedFloat<F> InnerProductCompensated(F a, F b, T... tail)
...
template <std::floating_point... T>
YOTH_HOST_DEVICE inline auto InnerProduct(T... values) {
...
template <std::integral T>
YOTH_HOST_DEVICE inline T InnerProduct(T a, T b) {
  return a * b;
}

template <std::integral F, std::integral... T>
YOTH_HOST_DEVICE inline F InnerProduct(F a, F b, T... tail) {
