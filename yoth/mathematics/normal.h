...
class Normal3 : public VectorType3<T> {
public:
  ...
  Normal3(U v) : base_type(T(v)) {
  }

  template <std::floating_point U>
  Normal3(U v0, U v1, U v2) : base_type(T(v0), T(v1), T(v2)) {
  }

  template <std::floating_point U>
  Normal3(const Vector3<U> &v) : base_type(T(v.x), T(v.y), T(v.z)) {
  }
};
